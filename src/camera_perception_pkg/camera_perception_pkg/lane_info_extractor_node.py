...
class Yolov8InfoExtractor(Node):
    def __init__(self):
        ...
        self.last_switch_time = 0.0  # 최초 전환 시각 (epoch time)
        self.cooldown_sec = 6      # 쿨타임 (초)
    ...
    def yolov8_detections_callback(self, detection_msg: DetectionArray):
        ...
        for det in detection_msg.detections:
            if det.class_name == 'road-objects':
                ...
                if obstacle_bottom_y is None or bottom > obstacle_bottom_y:
                    obstacle_bottom_y = bottom

        # Step 2: 조건 판단
        current_time = time.time()  # 현재 시간 (초 단위 float)
        if obstacle_bottom_y is not None and obstacle_bottom_y >= 320:
        ...
        lane.last_switch_time = self.last_switch_time        
